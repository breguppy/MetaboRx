...
#' 4) Separates blank/PB rows and detects metabolites below the blank threshold
#' 5) Optionally removes metabolites below the blank threshold
#' 6) Counts non-numerical and exact zeros that are replaced as missing values
#' 7) Makes sure injection order starts and ends with a QC sample
#' 8) Finds duplicate metabolite columns
#'
...
#' @param blank_threshold Numeric multiplier for blank threshold detection.
#'   A metabolite fails when QC mean is less than
#'   `blank_threshold * blank_mean`.
#' @param remove_blank_threshold_cols Logical. If TRUE, removes metabolites that
#'   fail blank threshold detection.
#' @param protect_internal_standards Logical. If TRUE, internal standards are not
#'   removed even if they fail the blank threshold.
#'
#' @return A list containing cleaned data, metadata, removed columns, replacement
#'   counts, blank threshold results, and duplicate metabolite information.
#'
#' @keywords internal
#' @noRd
clean_data <- function(df,
  ...
  qc_idx_norm <- tolower(class_chr) %in% "qc"
